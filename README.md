...
- 宛先指定Wi-Fiメッセージの `delivery_ack` + 自動再送（chat / long_text / image）
...
- 長距離寄りの既定プロファイル（LR有効、最大送信電力、再送/ジッタ最適化）
...
- COM接続/切断
- GUIからBuild / ファーム書き込み（選択COM / 全COM）
...
```powershell
cd D:\codebase\LPWAtestESP32
python tools/mesh_smoke_test.py --ports COM6 COM7 COM8 COM9 --timeout 35 --ack-timeout 4 --ack-retries 6
```
...
- 長距離寄り既定値は build flag で調整可能（`LPWA_ENABLE_WIFI_LR`, `LPWA_MESH_CHANNEL`, `LPWA_MESH_TX_POWER_QDBM`）
