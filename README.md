...
- 宛先指定Wi-Fiメッセージの `delivery_ack` + 自動再送（chat / image）
...
```powershell
cd D:\codebase\LPWAtestESP32
python tools/mesh_smoke_test.py --ports COM6 COM7 COM8 COM9 --timeout 35 --ack-timeout 12 --ack-retries 2
```

BLE短文試験をスキップしてWi-Fi系だけ確認する場合:

```powershell
python tools/mesh_smoke_test.py --ports COM6 COM7 COM8 --timeout 35 --ack-timeout 12 --ack-retries 2 --skip-ble
```
